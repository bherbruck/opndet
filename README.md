...
**Params + FLOPs for every bundled preset:** [`docs/models.md`](docs/models.md) (auto-generated by `scripts/model_table.py` — re-run it after adding/changing a preset). Note: the `-seg` presets show much higher FLOPs than their param count suggests because the dense head runs the last conv stages at full input resolution (16× the spatial area of a stride-4 head) — that's the cost the size ladder pays for exact per-object area.
