...
def test_noop_when_default():
    ...
    out_img, out_b, out_o, _ = _geometric(img.copy(), boxes.copy(), _cfg(), np.random.default_rng(0), obbs=None)
...
def test_zoom_out_keeps_all_boxes_shrunk():
    ...
    _, out_b, _, _ = _geometric(img.copy(), boxes.copy(), cfg, np.random.default_rng(0), obbs=None)
...
def test_obb_theta_invariant_wh_scaled_and_kept_in_sync():
    ...
    _, out_b, out_o, _ = _geometric(img.copy(), boxes.copy(), cfg, np.random.default_rng(0), obbs=obbs.copy())
...
def test_translate_only_shifts():
    ...
    _, out_b, _, _ = _geometric(img.copy(), boxes.copy(), cfg, np.random.default_rng(1), obbs=None)
