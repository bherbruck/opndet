...
### 2.2 Distance-transform output (replaces or augments objectness heatmap) — **SHIPPED (two forms)**

- **Detector form (stride 4, in-graph peak op kept):** `hm_target: ellipse` + `cls_loss: soft_hm` — the objectness heatmap target becomes a domed ellipse fitted to each box (1.0 at center, linear ramp to 0 at the inscribed-ellipse edge, exactly 0 in the corners / between touching objects), QFL-regressed so the model learns the *shape*. The §2.2 plan at the detector's native resolution; ships in every preset's training config. See `encode.py::_draw_rotated_dome` / `_render_dist_target`.
- **Full-resolution dense form:** the `bbox-{f,p,n,s,m}-seg` preset family — a U-Net-ish decoder that upsamples all the way back to input resolution and emits a single `[1, 1, H, W]` dome map. Not a detector (no boxes, no in-graph peak op — threshold + connected-components for instances/area; `decode.decode_seg`). The point: at stride 4 a 512×384 input gives a 128×96 dome, too coarse to read a small object's *area* off; at stride 1 it's exact. From per-instance masks → true per-instance L2-distance-transform dome (any convex shape); lacking masks → the elliptical dome from `opndet sam-obb` OBBs. `encode.encode_targets_seg`, `loss.SegDomeLoss` (QFL + soft Dice), `train_seg.py` (Dice/IoU/area/count metrics, auto-dispatched). TODO: `opndet sam-seg` mask exporter for the true distance-transform target on real masks.
