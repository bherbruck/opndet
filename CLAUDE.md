...
- `train.py` — training loop. TensorBoard is **off by default** (`tensorboard: true` to enable; the dashboard reads the DuckDB store, not tfevents) — and lazy-imported with a no-op-writer fallback when import fails (Colab numpy/tensorboard mismatches). Auto-increment `out_dir`, resume, trajectory-patience, curriculum w/ alias map, cosine LR + warmup. End-of-training Colab auto-download: `download: best|bundle|none` (default `best` = just `<name>_best.pt` + `metrics.duckdb`; `bundle` = zip the run dir minus `tb/` and `vis/` — ckpts + duckdb + config + eval reports, the vis PNGs are dashboard artifacts served live, not download-worthy; `bundle_include_vis`/`bundle_include_tb: true` add them back; back-compat: `auto_bundle: true/false` still maps to `bundle`/`none` if `download` is absent). Reads `optimizer: adamw|musgd` from config (default adamw).
