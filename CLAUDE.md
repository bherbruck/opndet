...
- `train.py` — training loop. TensorBoard is **off by default** (`tensorboard: true` to enable; the dashboard reads the DuckDB store, not tfevents) — and lazy-imported with a no-op-writer fallback when import fails (Colab numpy/tensorboard mismatches). Auto-increment `out_dir`, resume, trajectory-patience, curriculum w/ alias map, cosine LR + warmup. End-of-training Colab auto-download: `download: best|bundle|none` (default `best` = just `<name>_best.pt` + `metrics.duckdb`, not a GB-sized zip of the vis/ PNGs — those are dashboard artifacts; `bundle` = the old whole-run zip; back-compat: `auto_bundle: true/false` still maps to `bundle`/`none` if `download` is absent). Reads `optimizer: adamw|musgd` from config (default adamw).
