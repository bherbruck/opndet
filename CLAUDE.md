...
- `-obb` (`bbox-{f,p,n,s}-obb`) — base architecture (no SPPF/PAFPN/decoupled head) + 7-ch OBB head (`obj + ltrb + Tanh angle`). Edge tier, opset-13/Myriad-X safe. Leaner than `-pro` for tightest MCU footprint with orientation. Same OBB output contract as `-pro`; same encode/decode/loss path. Requires OBB sidecar labels from one of: `opndet sam-obb` (box-prompt SAM2, fit OBBs) or `opndet coco-to-obb` (skip SAM — fit OBBs directly to the COCO json's `segmentation` polygons/RLE; use this when your dataset already has masks, e.g. a Roboflow COCO-Segmentation export); and curriculum `w_wh: 0.1` from ep 0 to avoid sub-cell-precision stall.
- `-seg` (`bbox-{f,p,n,s,m}-seg`) — **dense full-resolution dome SEGMENTATION head, not a detector.** Backbone = the matching base preset; the decoder upsamples all the way back to input resolution (laterals from `stem` @ /2 and the raw input @ /1, three more nearest-upsample stages /4→/2→/1, a full-res conv head) and emits a single `[1, 1, H, W]` map — 1.0 at each convex object's deepest interior point, ~linear ramp to 0 at its boundary, exactly 0 between touching objects. No boxes, **no in-graph peak op** — you threshold + connected-components for instances/area (`decode.decode_seg`). Why: a stride-4 dome (128×96 for a 512×384 input) is too coarse to read a small object's *area* off; at stride 1 it's exact. **Why even tiny models do this well:** the dome reframes "detect + size" as "binary semantic segmentation of one class" — the model spends *zero* parameters on size (extent = pixel count of the CC blob, decoded, not regressed), so a 31K-param `bbox-f-seg` learns it in a handful of epochs and is scale-robust by construction (a huge egg and a tiny one both just "light up their pixels" — no w/h regression to extrapolate across scales, which is exactly where a small *detector* head smears). The flip side: it pays in FLOPs, not params (the full-res conv head — `bbox-f-seg` is ~8× the FLOPs of `bbox-f` det at the same param count). So the seg "ladder" buys edge crispness + distractor discrimination, NOT sizing capacity — start at `-f`/`-n`, go bigger only if those lag. Tier edge — every op is opset-13 + Myriad-safe; the cost is the full-res conv head. Detected as a seg head via the `dome` alias. Trained by the dedicated loop in `train_seg.py` (auto-dispatched from `opndet train` / `opndet.train.train()` — different metric space: Dice / IoU / per-object area & count, no calibration / curriculum / eval-threshold). GT: `encode.encode_targets_seg` renders the dense dome at `cfg.img_h/img_w // seg_stride`. GT shape source, priority order: **(1) per-instance masks** — either a `<stem>.png` instance-id label map from `opndet sam-seg` (`data.sources[*].mask_dir`; box-prompt SAM2 with the COCO GT AABBs, `clip_to_box` cuts any mask that escaped the prompt) OR **rasterized straight from COCO `segmentation`** (polygons via `cv2.fillPoly`; RLE — both the uncompressed list-counts and the compressed ASCII-string form — via the native `dataset._decode_coco_rle`, no `pycocotools` dep) if your dataset already has masks — `dataset.load_coco_single_class` reads `annotations[].segmentation` into `Sample.coco_segs`, `OpndetDataset._instance_label_map`/`_rasterize_coco_segs` rasterizes it (skipping rings whose coords are wildly out of bounds — a known Roboflow-export glitch); no `sam-*` preprocessing needed. This is the path for a Roboflow "COCO Segmentation" export (mostly RLE). **(2) fallback** = the elliptical `_draw_rotated_dome` from `opndet sam-obb` OBBs (`obb_dir`). Profile knobs (top-level train config): `seg_dome_ramp_px` (0 = proportional ramp; >0 = flat-top plateau, ~N-px edge dropoff — keep small, 1-2, so the decoded footprint sits at the true edge) . **Separating touching objects:** the preferred mechanism is the loss-side `seg_w_separation` term (below) — `encode_targets_seg` emits a `seam` mask (a ~few-cell band along every inter-instance contact line) and the loss pulls the *predicted* dome → 0 on it, so plain threshold+CC splits touchers and the GT footprint isn't eroded. The older `seg_instance_gap_px` (>0, mask GT only) instead *carves* that corridor out of the GT dome — same effect but it shrinks touching objects' GT footprint ~gap/2 px on the contact side, so prefer the loss term and leave the carve at 0. Decode mode: `seg_decode_mode: cc|watershed` (auto `cc` when `seg_dome_ramp_px>0` — a flat-top plateau is bumpy and watershed-from-peaks shatters one object into a Voronoi of wedges; `cc` = threshold + connected-components is the right decode and is immune to that). Masks ride through the dataloader's aug (flip/rot90/scale, NEAREST) + letterbox like OBBs; mosaic auto-disabled when masks are present. Loss: `loss.SegDomeLoss` (QFL toward the soft dome + squared-denominator soft Dice against the soft dome *ramp itself* — NOT a binarized `dome>thr` footprint, which would reward a flat-topped p≈1 over the whole object and fight the QFL ramp into a solid blob — + an optional `seg_w_separation` term: penalises `mean(p²)` over the `seam` mask (the inter-instance contact band `encode_targets_seg` emits) → pulls the predicted dome → 0 *between touching objects* (gradient 2p, bites hardest where a seam is bridged ~0.4). This is the preferred separation mechanism — it shapes the prediction, not the GT, so touching-object area accuracy is preserved (vs the older `seg_instance_gap_px` GT carve). Without any separation pressure a thin seam is a rounding-error fraction of QFL and the model floors it at ~0.4, which a low decode threshold re-merges.).
...
- `train.py` — training loop. TensorBoard is **off by default** (`tensorboard: true` to enable; the dashboard reads the DuckDB store, not tfevents) — and lazy-imported with a no-op-writer fallback when import fails (Colab numpy/tensorboard mismatches). Auto-increment `out_dir`, resume, trajectory-patience, curriculum w/ alias map, cosine LR + warmup. On start it writes the config into the run dir (`_persist_run_config`): `train.yaml` = the source file verbatim, `config.yaml` = the fully-merged spec that ran (preset defaults applied) — both ride along in the bundle. End-of-training Colab auto-download: `download: best|bundle|none` (default `best` = `<name>_best.pt` + `config.yaml` + `metrics.duckdb`; `bundle` = zip the run dir minus `tb/` and `vis/` — ckpts + duckdb + config + eval reports, the vis PNGs are dashboard artifacts served live, not download-worthy; `bundle_include_vis`/`bundle_include_tb: true` add them back; back-compat: `auto_bundle: true/false` still maps to `bundle`/`none` if `download` is absent). Reads `optimizer: adamw|musgd` from config (default adamw).
...
- `train_seg.py` — dedicated training loop for the `bbox-*-seg` (dense dome) head: Dice/fg-IoU/per-object area-MAPE/count-MAE metrics, QFL + soft-Dice (+ optional `seg_w_separation` seam-suppression) loss, EMA + cosine-LR; no calibration / curriculum / eval-threshold / temporal-prior (none apply). Auto-dispatched from `opndet train` and `opndet.train.train()` when the preset has `model.head: seg`. GT shapes (priority): per-instance masks — `data.sources[*].mask_dir` PNGs (`opndet sam-seg`) OR rasterized from the COCO json's `segmentation` polygons/RLE if present (`Sample.coco_segs`) — → true distance-transform dome; else `data.sources[*].obb_dir` (`opndet sam-obb`) → elliptical dome. Writes the same DuckDB store as `train.py` (`metrics_db: true` default): `train/loss`+`lr` streamed ~`steps_per_epoch//8` per epoch (flushed eagerly — `MetricsDB.add_scalar` buffers to 64 before writing+CHECKPOINTing, which the seg loop would otherwise never hit), `val/*` per epoch, `test/*` on new-best epochs. **Vis only on a new-best (or first/last) epoch** (`viz_only_on_improvement`, default true — same as the detector), for `vis_samples` val *and* test samples, under tags `val/seg` / `test/seg`: `sample_<i>_rgb.png` (clean RGB, written once) + four toggleable overlays per sample — `dome_pred`/`dome_gt` (TURBO heatmap of the dome; pred per-vis-epoch, gt once) and `seg_pred`/`seg_gt` (the *decoded* instance view from `decode_seg`: each blob filled with a distinct colour + a same-hue contour + a `<N>px` area label; `_render_seg_decoded`). No box rows — it's a dense-dome head, not a box model; per-blob extent is the `<N>px` label in the seg overlay, an AABB fitted around a dome blob is meaningless. The dashboard auto-discovers the overlay kinds; its overlay-opacity slider scales them all. `seg_fg_thresh` (default 0.5) = the dome foreground cut; `seg_edge_margin` (default 0; >0 e.g. 0.05) = frame-edge-touching (clipped) blobs are excluded from area-MAPE and tagged `·E` with a thin border. `evaluate_seg()` is reused by `opndet eval` for seg ckpts.
