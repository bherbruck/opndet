...
- `train_seg.py` — dedicated training loop for the `bbox-*-seg` (dense dome) head: Dice/IoU/area/count metrics, soft-Dice + QFL loss, dome-heatmap vis, EMA + cosine-LR; no calibration / curriculum / eval-threshold / repulsion / temporal-prior (none apply to a seg model). Auto-dispatched from `opndet train` and `opndet.train.train()` when the preset has `model.head: seg`. v1 trains the elliptical dome from `opndet sam-obb` sidecars (so it needs `data.sources[*].obb_dir`).
