...
- `predict.py` — single-image / video inference + visualization. For a `bbox-*-seg` ckpt (`opndet predict --model bbox-n-seg --ckpt … --image … --save vis.jpg`) it un-letterboxes the dense dome to the original image size, runs `decode_seg`, and writes a translucent dome heatmap + per-blob contour / centroid / pixel-area overlay; the returned JSON is `[{cx, cy, area_px, peak, x1,y1,x2,y2}, …]` (no `score`). `--threshold` becomes the dome foreground cut (default 0.5 for seg).
